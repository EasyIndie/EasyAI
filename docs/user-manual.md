...
```bash
# 对话列表
curl -H "Authorization: Bearer dev-key" http://localhost:3003/chat-api/conversations

# 创建对话
curl -X POST -H "Authorization: Bearer dev-key" http://localhost:3003/chat-api/conversations

# 删除对话
curl -X DELETE -H "Authorization: Bearer dev-key" http://localhost:3003/chat-api/conversations/<id>

# 获取消息历史
curl -H "Authorization: Bearer dev-key" http://localhost:3003/chat-api/conversations/<id>/messages
...
# 发送消息（流式）
curl -N -X POST -H "Authorization: Bearer dev-key" -H "Content-Type: application/json" \
  http://localhost:3003/chat-api/conversations/<id>/chat \
